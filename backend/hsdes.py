...
with open('qcoinConfig.json') as json_data:
...
"""
g = git.cmd.Git("/home/intel/gfx-driver")
g.pull()
...
def update_gitlog():
    gla.reload_log('gitlog.log')
    logger.info(str(datetime.datetime.now())+" finish reload")
...
class CompLoc(Resource):
    def get(self):
        ...
        results = []
        for path in paths:
            results.append(static_analysis_by_folder(
                path, args['date'], '/home/intel/test/'))
        return app.response_class(
